/**
 * @module usePaulyField
 * @description
 * The hook every PaulyForm UI component uses to connect to the `FormStore`.
 *
...
/**
 * The return value of `usePaulyField` — matches the shape expected by
 * every PaulyForm UI component.
 *
 * @typeParam TValue - The domain value type for this field
 */
export interface UsePaulyFieldReturn<TValue> {
...
/**
 * Per-field hook consumed by every PaulyForm UI component.
 *
 ...
 * @returns A `UsePaulyFieldReturn<TValue>` with value, error, ref, onChange, onBlur
 *
 * @example
 * ```tsx
 * function PaulyText({ name, ...rest }: PaulyTextProps) {
 *   const { value, error, ref, onChange, onBlur } = usePaulyField<string>(name);
 *
 *   return (
 *     <div>
 *       <input
 ...
 * ```
 */
export function usePaulyField<TValue = unknown>(
  name: string
): UsePaulyFieldReturn<TValue> {
  ...
  // displays errors (like <PaulyFieldError>) can subscribe to errors
