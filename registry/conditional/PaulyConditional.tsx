...
import type { PaulyConditionalProps } from './types';
 ...
 * ### How It Works
 *
 * 1. Subscribes to the `watch` field via `usePaulyField(watch)`.
 * 2. Evaluates `condition(field.value)`.
 * 3. Returns `<>{children}</>` if true, `null` if false.
 *
 * ### Zero Re-render Isolation
 *
 * Because `usePaulyField` uses `useSyncExternalStore` with path-scoped
 ...
 * >
 *   <PaulyText name="reason" label="Why?" />
 * </PaulyConditional>
 * ```
 */
export function PaulyConditional({
  watch,
  condition,
  children,
}: PaulyConditionalProps): React.JSX.Element | null {
  const field = usePaulyField(watch);
