...
import type { VRCameraProps } from './types';
import styles from './VRCamera.module.css';
...
export function VRCamera({
  ...
  className,
  id,
}: VRCameraProps): React.JSX.Element {
      ...
      if (videoRef.current) {
        ...
        videoRef.current.play().catch(err => {
          if (err.name !== 'AbortError') {
            console.error('[VRCamera] play() failed:', err);
          }
        });
      }
    } catch (err) {
      console.error('[VRCamera] Failed to access camera:', err);
    }
  }, [facingMode]);
