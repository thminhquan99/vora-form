...
import type { PaulyFieldErrorProps } from './types';
import styles from './PaulyFieldError.module.css';
 ...
 *
 * This component uses `usePaulyField(name)` but only reads the `error`
 ...
 */
export function PaulyFieldError({
  name,
  className,
  role = 'alert',
}: PaulyFieldErrorProps): React.JSX.Element | null {
  const { error } = usePaulyField(name);
