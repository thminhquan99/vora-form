...
import { useFormContext, usePaulyField } from '@pauly/core';
import type { UsePaulyTableReturn } from './types';

/**
 * Hook for array operations on a `<PaulyTable>` field.
 *
 * ### How It Works
 *
 ...
 * 3. This triggers `useSyncExternalStore` in `<PaulyTable>` → re-render
 ...
 * ### Re-render Contract
 *
 * | Action       | PaulyTable re-renders? | Existing cells re-render? |
 * |-------------|------------------------|--------------------------|
 * | append()    | ✅ Yes (row added)      | ❌ No                     |
 * | remove()    | ✅ Yes (row removed)    | ❌ No (re-indexed silently)|
 * | Cell typing | ❌ No                   | ❌ No (setSilentValue)    |
 *
 * @param name - The array field path (must match `<PaulyTable name>`)
 */
export function usePaulyTable(name: string): UsePaulyTableReturn {
  ...
  const append = useCallback(
    (item: Record<string, unknown>) => {
      ...
      // Update the array shape — this triggers PaulyTable re-render
  ...
  const remove = useCallback(
    (index: number) => {
      ...
      // Update array shape — triggers PaulyTable re-render
